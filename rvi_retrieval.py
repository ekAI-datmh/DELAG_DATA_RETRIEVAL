...
def export_sentinel1_rvi(sentinel_collection, big_folder, roi, image_name, roi_name, folder_name):
    ...
    for i in range(count):
        ...
        # Construct the expected final output path for the merged TIFF
        final_merged_tif_path = os.path.join(out_folder, f"{image_name}_{date_str}.tif")

        # Check if the merged file already exists
        if os.path.exists(final_merged_tif_path):
            print(f"Skipping download and merge for {date_str}, merged file already exists: {final_merged_tif_path}")
            continue
